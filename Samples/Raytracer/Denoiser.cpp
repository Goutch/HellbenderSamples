...
void Denoiser::blurIrradiance(Frame &frame, GBufferResources &gbuffer_resources) {
	...
	denoiser_resources.vertical_blur_instance.setUniform("frame", &frame, context.getCurrentFrame());
...
Denoiser::Denoiser(uint32_t history_count) : context(*Application::instance->getContext()) {
	...
	denoiser_resources.temporal_accumulation_shader->loadGLSL("shaders/denoising/temporal_accumulation.comp",SHADER_STAGE_COMPUTE);
	...
	//BLUR
	denoiser_resources.horizontal_blur_shader->loadGLSL("shaders/denoising/horizontal_blur.comp",SHADER_STAGE_COMPUTE);
	denoiser_resources.vertical_blur_shader->loadGLSL("shaders/denoising/vertical_blur.comp",SHADER_STAGE_COMPUTE);
